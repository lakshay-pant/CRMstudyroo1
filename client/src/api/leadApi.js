...
const addLeadUrl = 'http://localhost:5000/v1/leads';
	...
	return new Promise(async (resolve, reject) => {
		try {
			const result = await axios.get(
				'http://localhost:5000/v1/leads/all-leads',
				{
					headers: {
						Authorization: sessionStorage.getItem('accessJWT'),
					},
...
export const getAllUserSingleLead = (_id) => {
	return new Promise(async (resolve, reject) => {
		try {
			const result = await axios.get('http://localhost:5000/v1/leads/' + _id);
	...
	return new Promise(async (resolve, reject) => {
		try {
			const result = await axios.put(
				'http://localhost:5000/v1/leads/' + id,
				frmData,
				{
					headers: {
						Authorization: sessionStorage.getItem('accessJWT'),
...
export const DeleteAllUserLeadsTask = (id1, id2) => {
	return new Promise(async (resolve, reject) => {
		try {
			const result = await axios.delete(
				'http://localhost:5000/v1/leads/' + id1 + '/' + id2,
				{
					headers: {
						Authorization: sessionStorage.getItem('accessJWT'),
					},
